...
for GSM in `echo $control_GSMs`; do
	...
	while :; do
		...
		if [ $nQ -le 10 ]; then
			qsub -N "chP$GSM" -o $Logfile -e $Logfile -pe def_slot 4 -b y python $project_root/sra2bam_mismatch1.py --GSM_ID $GSM --Genome $Genome --project_root "$project_root" --bioproject $bioproject
			sleep 1
			break
		fi
	done
