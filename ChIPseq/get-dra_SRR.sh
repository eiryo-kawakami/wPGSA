...
srafile_out(){
  local outdir=${1}
  local q_id=${2}
  ls -F "${outdir}" | grep '/' | grep ${q_id} | while read dir; do
    ls "${outdir}/${dir}" | while read path; do
      if [ -f ${outdir}/${dir}/${path} ]; then
        mv "${outdir}/${dir}/${path}" "${outdir}"
      else
        ls "${outdir}/${dir}/${path}" | while read file; do
          mv "${outdir}/${dir}/${path}/${file}" "${outdir}"
        done
      fi
    done
    rm -fr "${outdir}/${dir}"
...
validate(){
  ...
  ls "${outdir}" | grep ${query_id} | while read fname ; do
  ...
  # put out sra files
  srafile_out "${output_directory_path}" "${query_id}"
...
if [ `echo $experiment_id | grep 'SRX'` ]; then
